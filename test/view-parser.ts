...
describe("parseView", () => {
    it("should find angular attributes", async () => {
        ...
        assert.equal(13, viewInfos.length);
        ...
        assert.ok(viewInfos[10] instanceof LoopEnd);
        assert.equal("maxlength", (<FilterExpression>viewInfos[11]).filterInput);
        assert.equal("formatNumber", (<FilterExpression>viewInfos[11]).filterName);
        assert.deepEqual(["'hex'"], (<FilterExpression>viewInfos[11]).filterParams);
        assert.ok(viewInfos[12] instanceof LoopEnd);
    });
});
